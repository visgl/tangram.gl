...
The renderer uses the Ocular/esbuild toolchain for its browser runtime and
embedded worker bundle. The layer package uses `ocular-bundle`, while
repository-wide building, linting, and testing are orchestrated by
`@vis.gl/dev-tools`. Package `dist/` directories are generated by the build and
are not committed.
